 ...
 * @param <C>
 */
public abstract class EmbeddableResourceAssemblerSupport<T, D extends ResourceSupport, C>
	...
	/**
	 * Create a wrapped representation of a collection, to be added to the
	 * _embedded Resources of the containing resource It relies on the embedded
	 * resource being annotated with {@link Relation} and the collection of
	 * embedded objects being annotated with {@link JsonUnwrapped} (see
	 * {@link BookResource#setEmbeddeds()})
	 */
	...
	/**
	 * Create a wrapped representation of a single object, to be added to the
	 * _embedded Resources of the containing resource
	 */
	...
	/**
	 * Create an empty main object wrapping a list of resources. This is the way
	 * HAL expects endpoint returning a collection works
	 */
	...
	public Resources<D> toEmbeddedList(Iterable<T> entities, String self) {
		final List<D> resources = toResources(entities);
		return new Resources<D>(resources, new Link(self, "self"), linkTo(controllerClass).withRel("All"));
	}

	public D toEmbedded(T entity, String self) {
		D resource = toResource(entity);
		resource.add(new Link(self, "self"));
		resource.add(linkTo(controllerClass).withRel("All"));
		return resource;
	}
